package com.milesstudios.aquietnight.reference;

/**
 * Created by Ryanm14 on 2/28/2015.
 */
public class SharedPref {
    public static final String COOKED_FOOD = "cooked_food";
    public static final String WOOD = "wood";
    public static final String LEAVES = "leaves";
    public static final String STONE = "stone";
    public static final String AMBER = "amber";
    public static final String DIRTY_WATER = "dirty_water";
    public static final String RAW_FOOD = "raw_food";
    public static final String BOILED_WATER = "boiled_water";
    public static final String APPLE = "apple";
    public static final String TIN = "tin";
    public static final String BISMUTH = "bismuth";
    public static final String AMETHYST = "amethyst";
    public static final String NICKEL = "nickel";
    public static final String PLAYER_HEALTH = "player_health";
    public static final String STONE_SWORD = "stone_sword";
    public static final String TIN_SWORD = "tin_sword";
    public static final String FOREST_TEMPLE = "forest_temple";
    public static final String TIN_ARMOR = "tin_armor";
    public static final String LEAF_ARMOR = "leaf_armor";
    public static final String VILLAGE = "village";
    public static final String LEAD = "lead";
    public static final String TANNERY = "tannery";
    public static final String SMELTERY = "smeltery";
    public static final String HIDE = "hide";
    public static final String BONES = "bones";
    public static final String LEATHER_TUNIC = "leather_tunic";
    public static final String LEAD_PICK = "lead_pick";
    public static final String LEAD_AXE = "lead_axe";
    public static final String LEAD_CHEST_PLATE = "lead_chest_plate";
    public static final String LEAD_SWORD = "lead_sword";
}
